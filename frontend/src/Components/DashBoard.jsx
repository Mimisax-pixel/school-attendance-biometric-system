...
const Dashboard = () => {
  ...
  const totalClasses = data.totalClassesHeld || 0;

  // Use the new classes-based attendance averages
  const totalAttendance =
    data.totalCoursesAttendanceAverage?.overallAverage || 0;

  const attendanceData =
    data.totalCoursesAttendanceAverage?.levelAverages?.map((item) => ({
      level: item.level,
      attendance: item.average,
      totalClasses: item.totalClasses,
    })) || [];

  // Get department attendance for selected level
  const selectedLevelData =
    data.departmentAttendanceAverage?.find(
      (item) => item.level === selectedLevel
    ) || {};

  const departmentAttendanceData =
    selectedLevelData.departments?.map((item) => ({
  ...
  return (
    ...
    >
      ...
      <div className="w-full px-3 sm:px-5 md:px-8 lg:px-10 py-8 sm:py-10">
        ...
        {/* Stat Cards */}
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-6 w-full mb-10">
          {[
            { label: "Total Students", value: totalStudents },
            { label: "Teachers", value: totalLecturers },
            { label: "Courses", value: totalCourses },
            { label: "Total Classes", value: totalClasses },
            { label: "Attendance", value: `${totalAttendance.toFixed(2)}%` },
          ].map((stat, i) => (
            <div
              key={i}
        ...
        <div className="w-full space-y-6">
          {/* Level Selector */}
          <div className="flex items-center gap-4 bg-white p-4 sm:p-5 rounded-xl shadow-md">
            <label
              htmlFor="level-select"
              className="text-lg font-semibold text-[#0A496D]"
            >
              Select Level:
            </label>
            <select
              ...
              className="px-4 py-2 rounded-lg border-2 border-[#0A496D] text-[#0A496D] font-semibold focus:outline-none focus:ring-2 focus:ring-[#0A496D] transition duration-300 cursor-pointer bg-white"
            >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8 w-full">
            ...
            <div className="bg-white w-full p-4 sm:p-6 rounded-xl shadow-md hover:shadow-lg transition duration-300">
              <h3 className="text-lg sm:text-xl font-bold text-[#0A496D] mb-4">
                Attendance by Department (Level {selectedLevel})
              </h3>
              {departmentAttendanceData.length > 0 ? (
                    ...
                    <BarChart data={departmentAttendanceData}>
                      ...
                      <Tooltip
                        ...
                        labelFormatter={(label) => {
                          const dept = departmentAttendanceData.find(
                            (d) => d.abbreviation === label
                          );
                          return dept
                            ? `${dept.abbreviation} (${dept.department})`
                            : label;
                        }}
                      />
                      <Bar
                ...
                <div className="h-64 sm:h-72 md:h-80 flex items-center justify-center">
                  <p className="text-gray-500 text-center">
                    No data available for Level {selectedLevel}
                  </p>
                </div>
              )}
            </div>
